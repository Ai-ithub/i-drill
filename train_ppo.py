...
class MLflowCallback:
    ...
    def __init__(self, model_name="ppo_drilling_env"):
            ...
            for info in locals_['infos']:
                if 'episode' in info:
                    ...
                    if len(self.episode_rewards) % 10 == 0:
                        if MLFLOW_AVAILABLE and mlflow_service:
                            try:
                                mean_reward = sum(self.episode_rewards[-10:]) / 10
                                mean_length = sum(self.episode_lengths[-10:]) / 10
                                mlflow.log_metrics({
                                    'mean_reward': mean_reward,
                                    'mean_episode_length': mean_length,
                                }, step=locals_.get('self', locals_).num_timesteps)
...
def main():
    ...
    # Start MLflow run
    run_name = f"ppo-training-{datetime.now().strftime('%Y%m%d-%H%M%S')}"
    ...
    try:
        ...
        model_path = model_dir / "ppo_drilling_env"
